...
import { timeSeries as fallbackTimeSeries, devices as fallbackDevices } from "../data/dashboard";
import type { TimePoint, Device } from "../data/dashboard";
...
function Overview() {
  const [timeSeries, setTimeSeries] = useState<TimePoint[]>(fallbackTimeSeries);
  const [devices, setDevices] = useState<Device[]>(fallbackDevices);
  const [fanOn, setFanOn] = useState(true);

  useEffect(() => {
    apiFetch<TimePoint[]>("/device/history").then((res) => {
    ...
    <div>
      ...
      <div className="kpi-grid">
        ...
        <KpiCard label="Fan Status" value={fanOn ? "ON" : "OFF"} accent={fanOn ? "accent" : "danger"}>
          <button className={`fan-toggle ${fanOn ? "" : "off"}`} onClick={() => setFanOn((v) => !v)}>
            Toggle
          </button>
        </KpiCard>
      </div>
